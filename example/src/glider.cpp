...
namespace gazebo {
    /// \brief A plugin to control the Glider.
    ...
    public:
        ...
        virtual void Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
            ...
            if (_model->GetJointCount() == 0) {
                std::cerr << "Invalid joint count, Glider plugin not loaded\n";
                return;
            }
            ...
            this->model->GetJointController()->SetPositionTarget(
                    this->joint1->GetScopedName(), 0);

            this->model->GetJointController()->SetPositionTarget(
                    this->joint2->GetScopedName(), 0);
            ...
            this->sub1 = this->node->Subscribe(topicName1,
                                               &GliderPlugin::OnMsgElevator, this);
            this->sub2 = this->node->Subscribe(topicName2,
                                               &GliderPlugin::OnMsgRudder, this);
        }
    ...
    private:
        ...
        void OnMsgElevator(ConstVector2dPtr &_msg) {
            this->model->GetJointController()->SetPositionTarget(this->joint1->GetScopedName(), _msg->x());
        }

        void OnMsgRudder(ConstVector2dPtr &_msg) {
            this->model->GetJointController()->SetPositionTarget(this->joint2->GetScopedName(), _msg->x());
        }
